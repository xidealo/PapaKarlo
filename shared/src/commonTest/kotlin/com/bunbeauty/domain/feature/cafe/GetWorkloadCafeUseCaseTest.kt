...
class GetWorkloadCafeUseCaseTest {
    ...
    private val useCase: GetWorkloadCafeUseCase = GetWorkloadCafeUseCase(
        cafeRepo = cafeRepo
    )
    ...
    @Test
    fun `invoke should return AVERAGE workload when cafe has AVERAGE workload`() = runTest {
        // Arrange
        val cafeUuid = "cafe1"
        everySuspend { cafeRepo.getCafeByUuid(cafeUuid) } returns getCafe(
            uuid = cafeUuid,
    ...
    @Test
    fun `invoke should return LOW workload when cafe has LOW workload`() = runTest {
        // Arrange
        val cafeUuid = "cafe1"
        everySuspend { cafeRepo.getCafeByUuid(cafeUuid) } returns getCafe(
            uuid = cafeUuid,
    ...
    @Test
    fun `invoke should return LOW workload when cafe is not found`() = runTest {
        // Arrange
